...
get_messages <- function(filename) {
	...
	entry_regex <- '([0-9]{2}[/]){2}[0-9]{2}[,]? ([0-9]{2}[:][0-9]{2})(( [-])|([:][0-9]{2}[:])) .+'
	...
	messages_regex <- '([0-9]{2}[/]){2}[0-9]{2}[,]? ([0-9]{2}[:][0-9]{2})(( [-])|([:][0-9]{2}[:])) .+[:] .+'
	temp_messages <- lines[grep(messages_regex, lines)]

	date_regex <- '([0-9]{2}[/]){2}[0-9]{2}[,]? ([0-9]{2}[:][0-9]{2})(( [-])|([:][0-9]{2}[:])) '
	user_regex <- '.+[:]'
	msg_regex  <- ' .+'
	...
	res <- data.frame(DATE=as.POSIXlt(dates, "%d/%m/%y, %H:%M", tz=Sys.timezone()), USER=users, MESSAGE=msgs)
	tbl_df(res)
}